...
```mermaid
flowchart TB
    Start([Team.run called]) --> PreHooks[Execute Pre-Hooks]
    PreHooks --> Reasoning{Reasoning<br/>enabled?}
    
    Reasoning -->|Yes| ReasoningProcess[Execute Reasoning Agent]
    Reasoning -->|No| BuildContext
    ReasoningProcess --> BuildContext[Build System and User Context]
    
    BuildContext --> ModelExec[Send to Model]
    ModelExec --> ModelDecision{Model<br/>Decision}
    
    ModelDecision -->|Direct Response| ProcessResponse[Process Response]
    ModelDecision -->|Tool Call| ExecuteTool[Execute Tool]
    ModelDecision -->|Delegate| DelegateMembers[Delegate to Members]
    
    ExecuteTool --> ModelExec
    
    DelegateMembers --> MemberExec[Members Execute Tasks]
    MemberExec --> MemberResults[Collect Member Results]
    MemberResults --> ModelExec
    
    ProcessResponse --> ParserModel{Parser Model<br/>configured?}
    ParserModel -->|Yes| ParseOutput[Parse with Parser Model]
    ParserModel -->|No| OutputModel
    ParseOutput --> OutputModel{Output Model<br/>configured?}
    
    OutputModel -->|Yes| GenerateOutput[Generate with Output Model]
    OutputModel -->|No| PostHooks
    GenerateOutput --> PostHooks[Execute Post-Hooks]
    
    PostHooks --> StoreSession[Store Session & Metrics]
    StoreSession --> End([Return TeamRunOutput])
...
7. **Response is processed** and parsed into an [`output_schema`](/concepts/input_output/team#structured-output) if provided.
...
See the [Input & Output](/concepts/input_output/overview) docs for more information, and to see how to use structured input and output with teams.
...
For more information see the [Team Sessions](/concepts/sessions/team) documentation.
