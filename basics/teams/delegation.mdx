...
```mermaid
flowchart LR
    Input[User Input] --> Leader[Team Leader<br/>Analyzes & Decides]
    Leader -->|Delegates| Member1[Member 1]
    Leader -->|Delegates| Member2[Member 2]
    Leader -->|Delegates| Member3[Member N]
    Member1 --> Results[Collect Results]
    Member2 --> Results
    Member3 --> Results
    Results -->|Synthesizes| Response[Final Response]
...
<Tip>
Use this feature with `determine_input_for_members=False` to create a **router pattern** — the team leader is effectively bypassed and all communication is directly with a team member.
</Tip>
...
Set `delegate_task_to_all_members=True` to delegate the task to **all members at once**, rather than selectively choosing members.
...
<CardGroup cols={3}>
  <Card title="Basic Coordination" icon="link" href="/basics/teams/usage/basic-flows/basic-coordination">
    Basic team coordination pattern
  </Card>
  <Card title="Router Team" icon="link" href="/basics/teams/usage/basic-flows/respond-directly">
    Router pattern with direct response
  </Card>
  <Card title="Cooperation Mode" icon="link" href="/basics/teams/usage/basic-flows/delegate-to-all-members">
